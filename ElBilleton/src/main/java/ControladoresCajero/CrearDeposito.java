...
public class CrearDeposito extends HttpServlet {
        ...
        try {
            ...
            if (monto != null && !"0.0".equals(monto) && !"".equals(monto)) {
                ...
                try {
                    ...
                    cuenta.setMonto(dinero + cuenta.getMonto());
                    Transaccion transaccionDeposito = new Transaccion(0, fecha, hora, "Credito", dinero, cuenta.getCodigo(), codigoCajero);
                    transaccionModel.agregarTransaccion(transaccionDeposito);
                    cuentamodel.modificarMonto(cuenta);
                    request.setAttribute("successDeposito", 1);
                    request.getRequestDispatcher("/cajero/RealizarDeposito.jsp").forward(request, response);
                } catch (Exception e) {
                }

            } else {
                request.setAttribute("successDeposito", 0);
                request.getRequestDispatcher("/cajero/RealizarDeposito.jsp").forward(request, response);
            }
        } catch (Exception e) {
